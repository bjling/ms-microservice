...
public class EurekaInstanceConfigBean implements CloudEurekaInstanceConfig, EnvironmentAware {
     ...
     *
     * 指示eureka客户端需要多长时间（以秒为单位）将心跳发送到eureka服务器，以指示它仍然活着。
     * 如果心跳在指定leaseExpirationDurationInSeconds期间内未被接收到，服务器将从视图中移除了实例，在这个实例通过禁止交易。
     *
     * 即发送心跳间隔为30s，如果90s内仍未发送到服务器，服务器则移除视图
     *
     * 注意，实例仍然不能承载交易如果它实现了HealthCheckCallback（废除了）并且决定自身不可用
     *
     * 租约续约间隔
     *
     */
