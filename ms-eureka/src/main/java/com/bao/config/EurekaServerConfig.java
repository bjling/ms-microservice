...
public interface EurekaServerConfig {
     ...
     *
     * 获取AWS Access Id 。主要用来Elastic IP Biding。它将提供合适的AWS许可来绑定EIP。
     *
     * @return
     */
     ...
     * appropriate AWS permissions to bind the EIP.
     *
     * 获取AWS Secret Key。主要用来Elastic IP Biding。它将提供合适的AWS许可来绑定EIP。
     *
     * @return
     */
     ...
     * EIP.
     *
     * 获取Eureka server 应该尝试获取候选EIP的次数（默认3次）
     *
     * <p>
     ...
     *
     * 获取Eureka server应该检验的时间间隔，已绑定的或者尝试去绑定，当且仅当当前未绑定的EIP（默认一分钟）
     * iff = if and only if
     *
     * <p>
     * <em>The changes are effective at runtime.</em>
     ...
     *
     * 获取Eureka server应该检验的时间间隔，已绑定的或者尝试去绑定，当且仅当当前绑定的EIP。（所以它仅为稳定状态检查）（默认5分钟）
     *
     * <p>
     * <em>The changes are effective at runtime.</em>
     * </p>
     ...
     *
     * 检查Eureka server是否开启self preservation自我保护模式（默认开启）
     *
     * <p>
     ...
     * </p>
     *
     * 当开启时，Eureka server保持renewals的数量的踪迹，它由server接收到的。
     * 任何时间，renewals的数量低于getRenewalPercentThreshold()阀值百分比，
     * Eureka server关闭过期来避免转移危险。它将帮助server来维护注册信息因为服务器与客户端之间的网络问题
     *
     * renewals  更新; 重建( renewal的名词复数 ); 重生; 合同的续订;  续约
     *
     ...
     *
     * renewals的最小百分比预计从服务端在周期内getRenewalThresholdUpdateIntervalMs()。
     * 如果renewals低于阀值，在自我保护模式下就不会过期（也就是说是0.85*15分钟，大概13分钟）
     *
     * <p>
     * <em>The changes are effective at runtime.</em>
     * </p>
     ...
     *
     * peer eureka nodes的信息更新时间间隔，用户可以使用DNS域名系统机制或者动态配置来动态改变信息（默认10分钟）
     *
     * <p>
     * <em>The changes are effective at runtime.</em>
     * </p>
     ...
     *
     * 如果设置为true, 发送请求中的复制的数据会一直被压缩。
     * 它不定义响应路径（被header Accept-Encoding所驱动）（默认false）
     *
     ...
     * updated.
     *
     * 获取peer nodes被更新的状态信息的时间间隔（默认30s）
     *
     * <p>
     * <em>The changes are effective at runtime.</em>
     * </p>
     ...
     *
     * 获取等待时间当Eureka server 启动无法从peer nodes获取实例。最好不要立即启动服务在这些情况下
     * 当信息在注册处存储未完成时。
     *
     * 当实例注册中心启动为空时，随着客户机开始发送心跳，服务器将请求客户端注册信息，它将随着时间的推移而生成。（默认5分钟）
     *
     ...
     * replication.
     *
     * 获取超时时间当为了复制 连接 peer eureka nodes（默认200ms）
     *
     ...
     * replication.
     *
     * 获取超时时间当为了复制 读取 peer eureka nodes（默认200ms）
     *
     * @return timeout value in milliseconds.
     */
     ...
     *
     * 获取http连接总数允许peer eureka nodes 复制 （默认1000）
     *
     ...
     *
     * 获取一个特别的http连接总数允许peer eureka nodes 复制 （默认500）
     *
     ...
     *
     * 获取将增量信息缓存到客户端以检索值而不丢失该值的时间。（3分钟）
     * @return time in milliseconds
     */
     ...
     *
     * 获取清理任务应该唤醒并检查过期delta信息的时间间隔。（默认30s）
     *
     * @return time in milliseconds.
     */
     ...
     *
     * 二级缓存策略来响应，读写缓存有效期限协定，只读缓存没有过期时间（true）
     *
     ...
     *
     * eureka节点在启动时从peers尝试获取注册信息的次数（5）
     *
     * @return the number of retries
     */
     ...
     *
     * eureka从peers尝试获取注册信息的等待时间间隔，如果前一次失败然后再次尝试（30s）
     *
     ...
     *
     * 获取允许在备份池中备份的复制事件的最大数目。此复制池负责除状态更新之外的所有事件。（10000）
     *
     * <p>
     * Depending on the memory allowed, timeout and the replication traffic,
     * this value can vary.
     ...
     *
     * 将此实例的可用对等复制实例的最小数目视为健康。eureka的设计允许一个实例继续使用零个对等点，但这并不理想。
     *
     * 默认值- 1被解释为一个标记，而不是比较副本的数量。这样做既可以禁用此检查，也可以在单个节点配置中运行eureka。
     *
