...
if [ -z ${ARTIFACT} ] || [ -z ${VERSION} ] || [ -z ${DESC} ] || [ -z ${GIT} ] ; then
    echo "require 4 params: Artifact, Version, Description, Git; options: Branch, PRO_DIR; default branch is master"
    exit -1
else
    echo "build params ARTIFACT: $ARTIFACT, VERSION: $VERSION, DESC: $DESC, GIT: $GIT, BRANCH: $BRANCH, TAG_NAME: $TAG_NAME, PRO_DIR: $PRO_DIR"
fi
